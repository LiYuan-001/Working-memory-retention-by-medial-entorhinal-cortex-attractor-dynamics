...
%
%          This next command is for a similar sample to the training
...
properties(Constant)
    ...
    methods(Static)
        ...
        function ok=NeedsPythonForTensorflow
            ok=MatBasics.usingOlderThan('R2021a');
        end
...
%
        ...
        function ok=IsVersionOk(version)
            A=strsplit(version, {'.', ' ', '\n', '\r'});
            ok=startsWith(lower(A(1)), 'python') && length(A)>2 ...
                && str2double(A(2))>=3 && str2double(A(3))>=7;
        end
       ...
       function [ok, version, cmd]=IsAvailable(app, interactWithUser, reset)
           ...
           if ok
                ...
                ok=MlpPython.IsVersionOk(version);
                if ok
